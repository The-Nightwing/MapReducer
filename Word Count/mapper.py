...
class Mapper(mapper_pb2_grpc.ServerServicer):
    ...
    def map(self, request, context):
        ...
        return master_pb2.MapperResponse(status='Mapper Finished')

    def partitonStrategy(self, keyVPairs):
...
def main(port):
    ...
    mapper_pb2_grpc.add_RegistryServerServicer_to_server(Mapper(name, port), server)
